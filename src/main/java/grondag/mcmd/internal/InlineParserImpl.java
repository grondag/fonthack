...
import grondag.mcmd.internal.inline.AsteriskDelimiterProcessor;
import grondag.mcmd.internal.inline.ObfuscateDelimiterProcessor;
import grondag.mcmd.internal.inline.StrikethroughDelimiterProcessor;
import grondag.mcmd.internal.inline.UnderlineDelimiterProcessor;
import grondag.mcmd.internal.inline.UnderscoreDelimiterProcessor;
import grondag.mcmd.internal.util.Escaping;
import grondag.mcmd.internal.util.Html5Entities;
import grondag.mcmd.internal.util.LinkScanner;
...
import grondag.mcmd.parser.InlineParserContext;
...
public class InlineParserImpl implements InlineParser {
    ...
    public InlineParserImpl(InlineParserContext inlineParserContext) {
        delimiterProcessors = calculateDelimiterProcessors(inlineParserContext.getCustomDelimiterProcessors());
    ...
    public static Map<Character, DelimiterProcessor> calculateDelimiterProcessors(List<DelimiterProcessor> delimiterProcessors) {
        final Map<Character, DelimiterProcessor> map = new HashMap<>();
        addDelimiterProcessors(Arrays.<DelimiterProcessor>asList(
                new AsteriskDelimiterProcessor(),
                new UnderscoreDelimiterProcessor(),
                new StrikethroughDelimiterProcessor(),
                new ObfuscateDelimiterProcessor(),
                new UnderlineDelimiterProcessor()), map);
    ...
    public void parse(String content, Node block) {
        ...
        Node previous = null;
        while (true) {
            final Node node = parseInline(previous);
            previous = node;
            if (node != null) {
    ...
    private Node parseInline(Node previous) {
        ...
        switch (c) {
            ...
            case '&':
                ...
                break;
            default:
                final boolean isDelimiter = delimiterCharacters.get(c);
                if (isDelimiter) {
        ...
        if (node != null) {
            ...
            final String literal = String.valueOf(c);
            return text(literal);
        }
    }
    ...
    private Node parseNewline(Node previous) {
        ...
        if (previous instanceof Text && ((Text) previous).getLiteral().endsWith(" ")) {
            final Text text = (Text) previous;
            final String literal = text.getLiteral();
            final Matcher matcher = FINAL_SPACE.matcher(literal);
            final int spaces = matcher.find() ? matcher.end() - matcher.start() : 0;
            if (spaces > 0) {
            ...
            // Grondag: extension - count blank line as hard break
            return previous instanceof SoftLineBreak ? new HardLineBreak() : new SoftLineBreak();
        }
    }
    ...
    private Node parseBackticks() {
        final String ticks = match(TICKS_HERE);
        if (ticks == null) {
            return null;
        }
        final int afterOpenTicks = index;
        String matched;
    ...
    private Node parseDelimiters(DelimiterProcessor delimiterProcessor, char delimiterChar) {
        final DelimiterData res = scanDelimiters(delimiterProcessor, delimiterChar);
        if (res == null) {
            return null;
        }
        final int length = res.count;
        final int startIndex = index;

        index += length;
        final Text node = text(input, startIndex, index);
    ...
    private Node parseCloseBracket() {
        ...
        // Get previous `[` or `![`
        final Bracket opener = lastBracket;
        if (opener == null) {
        ...
        if (!isLinkOrImage) {
            ...
            final int beforeLabel = index;
            parseLinkLabel();
            final int labelLength = index - beforeLabel;
            String ref = null;
            if (labelLength > 2) {
            ...
            if (ref != null) {
                final String label = Escaping.normalizeReference(ref);
                final LinkReferenceDefinition definition = context.getLinkReferenceDefinition(label);
                if (definition != null) {
    ...
    private String parseLinkDestination() {
        final int afterDest = LinkScanner.scanLinkDestination(input, index);
        if (afterDest == -1) {
            return null;
        }
    ...
    private String parseLinkTitle() {
        final int afterTitle = LinkScanner.scanLinkTitle(input, index);
        if (afterTitle == -1) {
            return null;
        }

        // chop off ', " or parens
        final String title = input.substring(index + 1, afterTitle - 1);
        index = afterTitle;
    ...
    int parseLinkLabel() {
        ...
        final int startContent = index + 1;
        final int endContent = LinkScanner.scanLinkLabelContent(input, startContent);
        // spec: A link label can have at most 999 characters inside the square brackets.
        final int contentLength = endContent - startContent;
    ...
    private Node parseAutolink() {
        ...
        if ((m = match(EMAIL_AUTOLINK)) != null) {
            final String dest = m.substring(1, m.length() - 1);
            final Link node = new Link("mailto:" + dest, null);
            node.appendChild(new Text(dest));
            return node;
        } else if ((m = match(AUTOLINK)) != null) {
            final String dest = m.substring(1, m.length() - 1);
            final Link node = new Link(dest, null);
            node.appendChild(new Text(dest));
            return node;
        } else {
            return null;
    ...
    private DelimiterData scanDelimiters(DelimiterProcessor delimiterProcessor, char delimiterChar) {
        ...
        final String before = startIndex == 0 ? "\n" :
            input.substring(startIndex - 1, startIndex);

        final char charAfter = peek();
        final String after = charAfter == '\0' ? "\n" :
            String.valueOf(charAfter);

        // We could be more lazy here, in most cases we don't need to do every match case.
        final boolean beforeIsPunctuation = PUNCTUATION.matcher(before).matches();
        final boolean beforeIsWhitespace = UNICODE_WHITESPACE_CHAR.matcher(before).matches();
        final boolean afterIsPunctuation = PUNCTUATION.matcher(after).matches();
        final boolean afterIsWhitespace = UNICODE_WHITESPACE_CHAR.matcher(after).matches();

        final boolean leftFlanking = !afterIsWhitespace &&
                (!afterIsPunctuation || beforeIsWhitespace || beforeIsPunctuation);
        final boolean rightFlanking = !beforeIsWhitespace &&
    ...
    private void processDelimiters(Delimiter stackBottom) {
        ...
        final Map<Character, Delimiter> openersBottom = new HashMap<>();
        ...
        while (closer != null) {
            ...
            final DelimiterProcessor delimiterProcessor = delimiterProcessors.get(delimiterChar);
            ...
            final Text openerNode = opener.node;
            final Text closerNode = closer.node;
    ...
    private void removeDelimitersBetween(Delimiter opener, Delimiter closer) {
        ...
        while (delimiter != null && delimiter != opener) {
            final Delimiter previousDelimiter = delimiter.previous;
